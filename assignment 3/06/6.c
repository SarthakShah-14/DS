...
void traversal()
{
	...
	{
		curr=first;
		printf("\nRNO\tNAME\tSEM\tMark1\tMark2\tMark3\TOTAl");
		while(curr!=NULL)
		{
			printf("\n%d\t%s\t%d\t%d\t%d\t%d\t%d",curr->rno,curr->name,curr->sem,curr->sub1,curr->sub2,curr->sub3,curr->total);
			curr=curr->next;
		}
	}
}
void deletefirst()
	...
	{
		tmp=first;
		printf("%d roll no is deleted",tmp->rno);
		if(first==last)
		{
			first=last=NULL;
		}
...
void deletelast()
{
	...
	{
		tmp=last;
		printf("%d rno is deleted",tmp->rno);
		if(first==last)
		{
			first=last=NULL;
		}
...
void deleteparticular()
{
	...
	{
		int Rno,cnt=1;
		printf("enter rolN you want to delete:");
		scanf("%d",&Rno);
		tmp=first;

		while(tmp->next!=NULL && tmp->rno!=Rno)
		{
			cnt++;
			curr=tmp;
			tmp=tmp->next;
